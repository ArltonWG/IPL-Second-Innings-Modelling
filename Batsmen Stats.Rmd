...
load(file='Player Roles.RData') #roles 
load(file='Player Splitter.RData') #batnames
start.time <- Sys.time()
...
stat_heads = c("...1","Player","MP","MB","NotOut","R","B","4s","6s","SR",
...
for (name in batnames$Player){
  batinfo = read_excel(paste0("Indiv Stats/Bat Stats/",name," Bat.xlsx"),sheet = "Bat")
    = profilered[tolower(profilered$Player) == tolower(name), "Position"]
...
save.image(file='Batsmen Stats.RData')

```

```{r loadstate}
#load(file='Batsmen Stats.RData')
